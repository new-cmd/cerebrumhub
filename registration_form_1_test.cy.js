 ...
 */
        ...
        // 26.10.2023 (User can submit data only when valid mandatory values are added)
describe('This is the first test suite, Andre Trahhov', () => {
    it('it.only', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Andre')
        cy.get('#lastName').type('Trahhov')
        cy.get('#username').type('Something')
        cy.get('input[name="password"]').type('@Andre_1212')
        cy.get('[name="confirm"]').type('@Andre_1212')
